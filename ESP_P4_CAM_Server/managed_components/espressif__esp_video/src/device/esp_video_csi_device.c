...
static bool csi_sensor_format_matches_v4l2_pixel_format(const esp_cam_sensor_format_t *sensor_format, uint32_t pixel_format)
{
    if (sensor_format->isp_info) {
        cam_ctlr_color_t out_color;
        uint8_t out_bpp;

        return csi_get_output_frame_type_from_v4l2(pixel_format, &out_color, &out_bpp) == ESP_OK;
    }

    uint32_t sensor_v4l2_format = 0;
    if (v4l2_get_input_frame_type_from_sensor(sensor_format->format, &sensor_v4l2_format) != ESP_OK) {
        return false;
    }

    return sensor_v4l2_format == pixel_format;
}

static bool csi_sensor_format_has_same_size(const esp_cam_sensor_format_t *lhs, const esp_cam_sensor_format_t *rhs)
{
...
static esp_err_t csi_video_enum_framesizes(struct esp_video *video, struct v4l2_frmsizeenum *frmsize, struct esp_video_stream *stream)
{
    ...
    ESP_RETURN_ON_ERROR(esp_cam_sensor_query_format(csi_video->cam.sensor, &format_array), TAG, "failed to query sensor formats");

    for (uint32_t i = 0; i < format_array.count; i++) {
        const esp_cam_sensor_format_t *sensor_format = &format_array.format_array[i];
        bool duplicated = false;

        if (!csi_sensor_format_matches_v4l2_pixel_format(sensor_format, frmsize->pixel_format)) {
            continue;
        }

        for (uint32_t j = 0; j < i; j++) {
            const esp_cam_sensor_format_t *prev_format = &format_array.format_array[j];

            if (!csi_sensor_format_matches_v4l2_pixel_format(prev_format, frmsize->pixel_format)) {
                continue;
            }

            if (csi_sensor_format_has_same_size(sensor_format, prev_format)) {
                duplicated = true;
                break;
            }
        }
        ...
        if (matched_index == (int)frmsize->index) {
            frmsize->type = V4L2_FRMSIZE_TYPE_DISCRETE;
            frmsize->discrete.width = sensor_format->width;
            frmsize->discrete.height = sensor_format->height;
            return ESP_OK;
        }
...
static esp_err_t csi_video_enum_frameintervals(struct esp_video *video, struct v4l2_frmivalenum *frmival, struct esp_video_stream *stream)
{
    struct csi_video *csi_video = VIDEO_PRIV_DATA(struct csi_video *, video);
    esp_cam_sensor_format_array_t format_array = {0};
    int matched_index = 0;
    ...
    for (uint32_t i = 0; i < format_array.count; i++) {
        ...
        if (!csi_sensor_format_matches_v4l2_pixel_format(sensor_format, frmival->pixel_format)) {
            continue;
        }

        if (sensor_format->width != frmival->width || sensor_format->height != frmival->height) {
            continue;
        }
        ...
        for (uint32_t j = 0; j < i; j++) {
            ...
            if (!csi_sensor_format_matches_v4l2_pixel_format(prev_format, frmival->pixel_format)) {
                continue;
            }

            if (prev_format->width == frmival->width &&
                prev_format->height == frmival->height &&
                prev_format->fps == sensor_format->fps) {
                duplicated = true;
                break;
            }
        }
        ...
        if (matched_index == (int)frmival->index) {
            frmival->type = V4L2_FRMIVAL_TYPE_DISCRETE;
            frmival->discrete.numerator = 1;
            frmival->discrete.denominator = sensor_format->fps;
            return ESP_OK;
        }
...
static const struct esp_video_ops s_csi_video_ops = {
    ...
    .enum_frameintervals = csi_video_enum_frameintervals,
#if ESP_VIDEO_ISP_DEVICE_CROP
